...
import { RequestingOfficerDashboard } from "./RequestingOfficerDashboard";
...
export const Dashboard = () => {
  ...
  useEffect(() => {
    if (user?.role === "REQUESTING_OFFICER") return;
  ...
  if (user?.role === "REQUESTING_OFFICER")
    return <RequestingOfficerDashboard />;

  if (loading) {
    return (
      <Loading
        type="ai"
        text="Fetching National Material Master Live Intelligence..."
      />
    );
  }
  ...
  return (
    <div className="space-y-6 bg-micro-dot p-2 sm:p-4 rounded-2xl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-3xl font-extrabold text-[#2f261f] tracking-tight">
            Material Intelligence
          </h2>
          <p className="text-sm text-[#6e625b] mt-1 font-medium">
            National Unified Material Master Live Overview
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <StatCard
          ...
          isGreenBorder={true}
          statusBadge={{
            text: `${stats?.nationalMaterials?.value ?? 0} Standardized`,
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          [
            "National harmonization index",
            `${stats?.harmonization?.index ?? 0}/100`,
          ],
          ["Verified procurement savings", "Not yet measured"],
          [
            "Surplus by unit (current month)",
            (stats?.stockAnalytics?.by_unit || [])
              .map((row) => `${row.surplus} ${row.uom}`)
              .join(" / ") || "No stock",
          ],
          [
            "Approval turnaround",
            `${stats?.approvalTurnaroundHours ?? 0} hours`,
          ],
        ].map(([label, value]) => (
          <div
            key={label}
            className="rounded-xl border border-[#e7dccd] bg-[#fffaf5] p-4"
          >
            <div className="text-[10px] font-bold uppercase tracking-wider text-[#7a6d63]">
              {label}
            </div>
            <div className="mt-1 text-xl font-black text-[#2f261f]">
              {value}
            </div>
          </div>
        ))}
        ...
        <div className="xl:col-span-4 flex flex-col gap-6">
          ...
          <div className="bg-[#fffaf5] border border-[#e7dccd] rounded-2xl p-6 ai-glow ai-border-gradient relative shadow-[0_1px_0_rgba(59,44,35,0.06)]">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <h3 className="text-base font-bold text-[#2f261f]">
            ...
            <div className="mb-5">
              <div className="flex justify-between items-end mb-2">
                <span className="text-xs font-medium text-[#6e625b]">
                  Harmonization Progress
                </span>
                <span className="text-lg font-black text-[#6c4738]">
                  {harmonizationRate}%
                </span>
              </div>
              <div className="w-full bg-[#efe3d7] rounded-full h-2 overflow-hidden">
                <div
                  className="bg-[#7d8f72] h-2 rounded-full transition-all duration-500"
                  style={{ width: `${harmonizationRate}%` }}
                ></div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="bg-[#f8e9e5] border border-[#ebcfc4] p-3 rounded-xl flex items-start gap-3">
                ...
                <div>
                  <span className="text-xs font-bold text-[#8f544e] block">
                    Review Queue
                  </span>
                  <span className="text-[12px] text-[#9d645d] font-medium leading-snug">
                    {pendingCount} candidate matches awaiting procurement
              ...
              <div className="bg-[#edf3ea] border border-[#d8e4d4] p-3 rounded-xl flex items-start gap-3">
                ...
                <div>
                  <span className="text-xs font-bold text-[#536d50] block">
                    Unified National Master
                  </span>
                  <span className="text-[12px] text-[#60775d] font-medium leading-snug">
                    {mappedCount} material records actively harmonized across
                    participating CPSEs.
                  </span>
                </div>
              </div>
