...
export const AIRecommendations = () => {
  ...
  const handleSearch = async (event) => {
    ...
    setSimilarResults([]);
    try {
      const response = await materialApi.searchDiscoverableMaterials(term, 25);
      setSearchResults(response.data || []);
    } catch (err) {
      setSearchResults([]);
      setError(
        err?.response?.data?.detail || err.message || "Material search failed.",
      );
    } finally {
      setSearching(false);
    }
    ...
    try {
      ...
      setError(
        err?.response?.data?.detail ||
          err.message ||
          "Unable to find similar materials.",
      );
    } finally {
      setMatching(false);
    }
  ...
  const createDirectRequest = async (event) => {
    ...
    if (
      ...
      setError(
        "Choose a requesting CPSE and enter a quantity greater than zero.",
      );
      return;
    }
    ...
    try {
      ...
      setRequestMaterial(null);
      navigate("/requests", {
        state: { createdRequestId: response.data.request.id },
      });
    } catch (err) {
      setError(
        err?.response?.data?.detail ||
  ...
  const compare = (targetCode) => {
    navigate(
      `/comparison?codeA=${encodeURIComponent(selected.code)}&codeB=${encodeURIComponent(targetCode)}`,
    );
  };
  ...
  const submitForApproval = async (result) => {
    ...
    setError("");
    try {
      const response = await matchingApi.submitForApproval(
        selected.id,
        result.material.id,
      );
      ...
      setError(
        err?.response?.data?.detail ||
          err.message ||
          "Unable to submit this match for approval.",
      );
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold tracking-tight text-slate-900">
            AI Similar Material Search
          </h2>
        </div>
        <p className="mt-1 text-xs text-slate-500">
          Search by material code, product name, description, category,
          manufacturer, or model, then find the closest compatible products.
        </p>
      </div>
            ...
            {searchResults.map((material) => (
              ...
              >
                <div className="mb-2 flex items-center justify-between gap-3">
                  <span
                    className={`rounded border px-2 py-0.5 text-[10px] font-bold ${getCPSEBadgeColor(material.cpse)}`}
                  >
                    {material.cpse}
                  </span>
      ...
      {matching ? (
        <Loading
          type="ai"
          text={`AI is finding products similar to ${selected?.code || "the selected material"}...`}
        />
        ...
        <section className="space-y-3">
          <div>
            <h3 className="text-sm font-bold text-slate-900">
              Top similar products for {selected.code}
            </h3>
            <p className="text-xs text-slate-500">
              Only compatible products that passed AI matching are shown.
            </p>
          </div>
                ...
                <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                  <div className="flex min-w-0 gap-3">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span
                          className={`rounded border px-2 py-0.5 text-[10px] font-bold ${getCPSEBadgeColor(result.material.cpse_code)}`}
                        >
                          {result.material.cpse_code}
                        </span>
                        <span className="font-mono text-xs font-bold text-blue-700">
                          {result.material.material_code}
                        </span>
                        <span
                          className={`rounded-full border px-2 py-0.5 text-[10px] font-bold ${scoreColor(result.score)}`}
                        >
                          {result.score}% match
                        </span>
                      </div>
                  ...
                  <div className="flex shrink-0 flex-wrap gap-2">
                    <Button
                      ...
                      size="sm"
                      icon={GitCompare}
                      onClick={() => compare(result.material.material_code)}
                    >
                      Compare
                    </Button>
                    {canSubmit && (
                      ...
                      >
                        {submittedIds.has(result.material.id)
                          ? "Submitted"
                          : "Submit for Approval"}
                      </Button>
                    )}
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Create a draft reservation request for the selected mapped
                  material.
                </p>
              </div>
              <button
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="text-xs font-bold text-slate-700">
                ...
                <select
                  required
                  value={requestingCpse}
                  onChange={(event) => setRequestingCpse(event.target.value)}
                  className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal"
                >
                  <option value="">Select CPSE</option>
                  {requestCpses.map((cpse) => (
              ...
              <label className="text-xs font-bold text-slate-700 sm:col-span-2">
                ...
                <input
                  required
                  value={requestUom}
                  onChange={(event) => setRequestUom(event.target.value)}
                  className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal"
                />
              </label>
            </div>
            <p className="mt-3 text-[11px] text-slate-500">
              The request stays in Draft status. You can review and submit it
