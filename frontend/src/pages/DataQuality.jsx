    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-slate-500">
          Live completeness, classification and harmonization metrics
        </p>
      </div>
      {error && (
      ...
      {metrics && (
        <>
          <div className="bg-slate-900 text-white rounded-2xl p-6 flex justify-between">
            <div>
              <div className="text-xs text-cyan-300 uppercase font-bold">
                Overall score
              </div>
              <div className="text-5xl font-black mt-2">
                {Number(metrics.data_quality_score || 0)}%
              </div>
              <div className="text-xs text-slate-400">
                {Number(metrics.total_materials || 0)} materials
              </div>
            </div>
          ...
          <section className="rounded border bg-white p-4">
            <h3 className="font-bold">Technical evidence to complete</h3>
            <p className="text-xs">
              First 100 records requiring attributes or a category schema.
            </p>
            {(metrics.technical_issues || []).map((row) => (
              <div key={row.material_id} className="border-t py-2 text-xs">
                <strong>{row.material_code}</strong>:{" "}
                {row.schema_supported
                  ? row.missing_attributes.join(", ")
                  : "Category schema requires engineering definition"}
              </div>
            ))}
          </section>
        </>
