...
const allNationalMaterials = async () => {
  ...
  for (let offset = 0; ; offset += 500) {
    const response = await api.get("/api/national-materials", {
...
export const nationalMaterialApi = {
  getNationalMaterials: async (params = {}) => {
    ...
    if (params.search) {
      ...
      resList = resList.filter(
        (x) =>
          x.nationalCode?.toLowerCase().includes(q) ||
          x.standardTitle?.toLowerCase().includes(q),
      );
    }
    return { data: resList };
  },

  createNationalMaterial: (d) =>
    api.post("/api/national-materials", {
      ...
      specifications:
        d.specifications && typeof d.specifications === "object"
          ? d.specifications
          : d.specifications
            ? { text: d.specifications }
            : {},
    }),

  deleteNationalMaterial: (id) => api.delete(`/api/national-materials/${id}`),
  addMapping: (nationalId, materialId) =>
    api.post(`/api/national-materials/${nationalId}/mappings/${materialId}`),
  removeMapping: (nationalId, materialId) =>
    api.delete(`/api/national-materials/${nationalId}/mappings/${materialId}`),

  getNationalMaterialById: (id) => api.get(`/api/national-materials/${id}`),
  getAvailability: (id) =>
    api.get(`/api/national-materials/${id}/availability`),
  ...
  getApprovals: async (
    ...
    return {
      data: (payload.items || []).map((x) => {
        ...
        const p =
          Number(x.final_score || 0) *
          (Number(x.final_score || 0) <= 1 ? 100 : 1);
        return {
          id: x.id,
      ...
      classificationCounts: payload.classification_counts || {
        EXACT: 0,
        NEAR_DUPLICATE: 0,
  ...
  approveMapping: (
    ...
    comment = null,
    canonicalValues = {},
    acknowledgeCriticalConflicts = false,
    acknowledgeFunctionalEquivalent = false,
  ) =>
    api.post(`/api/approvals/${id}/approve`, {
      comment,
      canonical_values: canonicalValues,
      acknowledge_critical_conflicts: acknowledgeCriticalConflicts,
      acknowledge_functional_equivalent: acknowledgeFunctionalEquivalent,
    }),
  rejectMapping: (id, reason) =>
    api.post(`/api/approvals/${id}/reject`, { comment: reason || null }),

  getAuditTrail: async (filters = {}) => {
    const r = await api.get("/api/audit");
    let l = (r.data || []).map((x) => {
      const details =
        typeof x.details === "object" && x.details ? x.details : {};
      const msg =
        details.message ||
        (typeof x.details === "string"
          ? x.details
          : `${x.action} event on ${x.entity_type || "Entity"} #${x.entity_id ?? ""}`);
      return {
        id: x.id,
        ...
        materialCode:
          ...
          `${x.entity_type || "Entity"} #${x.entity_id ?? "-"}`,
        details: msg,
      };
    });
    ...
    if (filters.search) {
      ...
      l = l.filter((x) =>
        `${x.details} ${x.action} ${x.materialCode} ${x.user}`
          .toLowerCase()
          .includes(q),
      );
    }
