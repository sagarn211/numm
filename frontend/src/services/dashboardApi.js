...
export const dashboardApi = {
  getStats: async () => {
    ...
    return {
      data: {
        totalCPSEs: stat(
          d.total_cpses ?? d.participating_cpses,
          "Participating CPSEs",
        ),
        totalMaterials: stat(
          d.total_materials ?? d.total_legacy_materials,
          "Total Material Items",
        ),
        duplicateMaterials: stat(
          d.duplicate_candidate_materials ?? d.pending_matches,
          `${d.duplicate_risk_percent ?? 0}% duplicate risk`,
        ),
        aiMatches: stat(d.mapped_materials, "Mapped Materials"),
        nationalMaterials: stat(
          d.total_national_materials ?? d.national_materials,
          "National Codes Created",
        ),
  ...
  getMaterialNetwork: async () => {
    ...
    const detailedNodes = await Promise.all(
      nationals.slice(0, 5).map(async (n) => {
        ...
        const linkedCpses = (detailRes.data?.legacy_mappings || []).map(
          (m) => ({
            cpse: m.cpse_code,
