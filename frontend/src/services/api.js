...
api.interceptors.response.use(r => r, e => {
  if (e.response?.status === 401) {
    ...
    localStorage.removeItem('numm_token'); localStorage.removeItem('numm_user');
    if (hadSession) {
      sessionStorage.setItem('numm_session_message', 'Your session expired. Please sign in again.');
