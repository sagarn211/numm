...
export const Sidebar = ({ collapsed, onToggle }) => {
  ...
  const navItems = [
    ...
    },
    {
      ...
      path: "/requests",
      icon: ClipboardList,
      anyOf: ["request.read", "request.read_all"],
    },
    {
      label: "CPSE Management",
      path: "/cpses",
      icon: Building2,
      permission: "cpse.manage",
    },
    {
      label: "Data Quality",
      path: "/data-quality",
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          {!collapsed && (
            <div className="flex flex-col leading-tight">
              <span className="font-extrabold text-xs tracking-wider text-[#f8f2eb] uppercase">
                NATIONAL
              </span>
              <span className="font-bold text-[10px] text-[#d6b79f] tracking-widest uppercase">
                MATERIAL MASTER
              </span>
              <span className="text-[9px] text-[#d7c6b7] font-mono">
                GRID V2.4
              </span>
            </div>
          )}
      ...
      <div className="flex-1 py-4 px-2 space-y-1 overflow-y-auto">
        <div
          className={`px-3 py-1 text-[10px] font-bold text-slate-500 uppercase tracking-wider ${collapsed ? "text-center" : ""}`}
        >
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon
                className={`w-4 h-4 shrink-0 transition-transform group-hover:scale-110 ${isActive ? "text-[#e9c9a4]" : "text-[#d9c7b7]"}`}
              />
              {!collapsed && (
      ...
      <div className="p-3 border-t border-[#4b3a30] bg-[#1F1A17]">
        {!collapsed ? (
          <div className="space-y-2 mb-3">
            <div className="text-[10px] font-bold text-[#c6b7a8] uppercase tracking-wider">
              SYSTEM STATUS
            </div>
            <div className="space-y-1.5 text-[11px] bg-[#332c29] p-2.5 rounded-lg border border-[#4b3a30]">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`flex items-center gap-1 font-semibold text-[10px] ${apiOnline ? "text-[#a7d1a4]" : "text-[#e8b1a5]"}`}
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${apiOnline ? "bg-[#a7d1a4] animate-pulse" : "bg-[#e8b1a5]"}`}
                  ></span>
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`flex items-center gap-1 font-semibold text-[10px] ${aiOnline ? "text-[#a7d1a4]" : "text-[#e8b1a5]"}`}
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${aiOnline ? "bg-[#a7d1a4] animate-pulse" : "bg-[#e8b1a5]"}`}
                  ></span>
          ...
          <div className="flex flex-col items-center gap-2 mb-2">
            <span
              title={apiOnline ? "API Connected" : "API Disconnected"}
              className={`w-2 h-2 rounded-full ${apiOnline ? "bg-[#a7d1a4] animate-pulse" : "bg-[#e8b1a5]"}`}
            ></span>
            <span
              title={aiOnline ? "AI Engine Online" : "AI Engine Offline"}
              className={`w-2 h-2 rounded-full ${aiOnline ? "bg-[#a7d1a4]" : "bg-[#e8b1a5]"}`}
            ></span>
          </div>
        )}
