...
export const StatCard = ({
  ...
  let containerClasses =
    "bg-[#fffaf5] border rounded-xl p-4 flex flex-col justify-between transition-all duration-200 shadow-[0_1px_0_rgba(59,44,35,0.06)] hover:shadow-md ";

  if (isAiGlow) {
    containerClasses += "ai-border-gradient ai-glow ";
  } else if (isRedAlert) {
    containerClasses +=
      "border-[#e6d8ce] border-r-4 border-r-[#b75d50] hover:border-[#d9c0b5] ";
  } else if (isGreenBorder) {
    containerClasses +=
      "border-[#e7e0d8] border-l-4 border-l-[#6b8d63] hover:border-[#d9c8ba] ";
  } else {
  ...
  return (
    <div className={containerClasses}>
      <div className="flex justify-between items-start">
        <span className="text-[11px] font-bold text-[#715f57] uppercase tracking-wider">
          {title}
        </span>
        {Icon && (
          <div
            className={`p-1.5 rounded-lg border ${iconVariants[accentColor] || iconVariants.blue} ${isAiGlow ? "pulse-border" : ""}`}
          >
      ...
      <div className="mt-3">
        ...
        <div className="flex items-center gap-1.5 mt-1.5 text-[11px] font-medium">
          {statusBadge ? (
            <span
              className={`inline-flex items-center gap-1 font-semibold ${statusBadge.color}`}
            >
            ...
            >
              ...
              <span>{change}</span>
              <span className="text-[#7a6d63] font-normal ml-0.5">
                {period}
              </span>
            </div>
          ) : (
