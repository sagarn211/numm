...
export const ImportStatus = ({
  ...
  const currentStageIndex = terminal
    ? stages.length
    ...
    <div className="bg-gradient-to-br from-[#0B1220] via-[#0F172A] to-[#1E1B4B] rounded-2xl p-6 border border-slate-800 text-white shadow-xl space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div>
          ...
          <h3 className="text-lg font-extrabold text-white tracking-tight mt-1.5">
            Processing CPSE{" "}
            {importType === "INVENTORY" ? "Inventory Stock" : "Master"} Dataset
          </h3>
        </div>
        <div className="text-right">
          ...
          <p className="text-[11px] text-slate-400 font-mono">
            Row {progressData.processedRows} / {progressData.totalRows}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-2 relative">
        {stages.map((stage, idx) => {
          ...
          return (
            ...
            >
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold mb-2 transition-all ${
                  isDone
              ...
              <span
                className={`text-[10px] font-extrabold tracking-wider uppercase font-mono ${
                  isActive
                    ? "text-cyan-400"
                    : isDone
                      ? "text-emerald-400"
