...
AI makes recommendations. A human reviewer remains responsible for the final harmonization decision, especially for functional equivalents and safety-critical differences.
...
The backend stores recommendations as `MaterialMatch` records. The page can filter by confidence and status. **Duplicate Clusters** (`/duplicate-clusters`) groups related candidates so reviewers can see a broader duplicate pattern instead of only one pair at a time. **Material Comparison** (`/comparison`) lets a user compare two selected records directly. **Model Evaluation** (`/model-evaluation`) shows the AI service evaluation response.

### Step 6: Review and approve mappings

Open **Approvals** (`/approvals`) as a user with `approval.review`. The queue supports pending, approved, and rejected tabs, classification filters, confidence sorting, pagination, and a side-by-side review modal.

The reviewer checks the source descriptions, category, scores, explanation, and conflicts. Safety-critical conflicts such as grade, pressure, voltage, or dimensions must be resolved by selecting the authoritative value. A `FUNCTIONAL_EQUIVALENT` recommendation requires explicit acknowledgement and substitution conditions. The reviewer can approve or reject; the backend records the decision and audit information.

This is a maker-checker boundary: the AI cannot silently create a canonical identity for an uncertain match.

### Step 7: Create and inspect national material identities
...
### Step 8: Use the unified data operationally
...
### Step 9: Synchronize with ERP and close the audit loop
...
| Duplicate Clusters | `/duplicate-clusters` | Groups related duplicate candidates. | Review clusters by status and understand consolidation scope. |
...
| Approvals | `/approvals` | Human governance queue for AI mappings and request decisions. | Review conflicts, approve/reject mappings, and handle request decisions. |
...
- Approval status is tracked as pending, approved, or rejected.
- Reviewers must resolve safety-critical conflicts before approval.
- Functional-equivalent mappings require acknowledgement and conditions.
- Approval and rejection actions are audited.
- Maker-checker rules separate request creation from approval where required.
...
The most important persistence boundaries are the import batch, material, match, approval/mapping, national material, inventory, request, procurement, integration sync, and audit records. When debugging a feature, trace the record through those boundaries instead of checking only the page that displayed it.
...
7. For an incorrect national mapping, inspect the approval history and mapping history before changing data manually.
